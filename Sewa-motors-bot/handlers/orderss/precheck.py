...
import asyncio
import config
from utils.data import get_order_by_id
...
@router.message(OrderStates.precheck_wait_manager, F.text & ~F.text.startswith('/'))
async def precheck_send_to_manager(message: Message, state: FSMContext):
    ...
    if not admin_id:
        ...
        await state.clear()
        return
    chat_order_id = _chat_manager_to_order.get(getattr(message.from_user, 'id', 0))
    if chat_order_id:
        try:
            await message.bot.send_message(
                admin_id,
                f"💬 Сообщение от осмотрщика:\n{message.text}",
                reply_markup=get_precheck_after_video_keyboard(chat_order_id),
            )
        except Exception:
            pass
        return
    if _require_video_for_order.get(order_id, False):
        await message.answer("⚠️ Требуется короткое видео. Пожалуйста, отправьте видеосообщение.")
        return
    consult_text = (
...
async def precheck_chat_start(callback: CallbackQuery, state: FSMContext):
    ...
    await state.update_data(chat_order_id=order_id)
    await callback.message.answer("✍️ Напишите сообщение осмотрщику одним сообщением. Он сможет ответить.")
    await callback.answer()
...
async def precheck_chat_bridge(message: Message, state: FSMContext):
    ...
    await message.bot.send_message(
        ...
        reply_markup=get_precheck_manager_reply_keyboard(str(order_id)),
    )
